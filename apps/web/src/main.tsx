...
// @ts-ignore
import './styles.css';
import { normalizeForLanguage, getSpeechCode, hasNativeScript, isInScript, LANGUAGE_CONFIG } from './languageNormalization';
...
function PatientView({ p, data, tab, setTab, action, setNotice, language, setLanguage, refresh, profileOpen, setProfileOpen }: any) {
    ...
    // voiceRecognitionInstance holds the active SpeechRecognition object so we can stop it on demand
    ...
    // speechLanguageMap is now provided by the centralised languageNormalization utility
    // getSpeechCode(language) → BCP-47 locale; normalizeForLanguage(text, language) → target script
    ...
    // Voice input: toggle start/stop, full language support, interim results, append behavior
    const SpeechRecognitionAPI = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
    const voiceSupported = !!SpeechRecognitionAPI;
    const toggleVoiceInput = () => {
      if (!voiceSupported) {
        setVoiceError('Voice input is not supported in this browser. Please type your symptoms manually.');
        return;
      }
      // If already listening: stop the current session
      if (listening && voiceRecognitionInstance) {
        try { voiceRecognitionInstance.stop(); } catch {}
        return;
      }
      ...
      // Use the centralised utility for the BCP-47 speech code
      recognition.lang = getSpeechCode(language);
      recognition.continuous = false;
      recognition.interimResults = true;
      // Capture the text already in the textarea at session start
      const baseText = symptoms;
      ...
      recognition.onresult = (event: any) => {
        ...
        if (finalText) {
          setInterimTranscript('');
          // Normalize: if the browser returned Roman text for an Indic language,
          // convert it to the target script. If it already IS the target script
          // (which modern Chrome/hi-IN returns directly), this is a no-op.
          const normalizedText = normalizeForLanguage(finalText.trim(), language);
          const trimmedBase = baseText.trim();
          const trimmedNew = normalizedText.trim();
          if (trimmedBase) {
            // Append with newline, avoid duplicate separators
            const separator = trimmedBase.endsWith('\n') ? '' : '\n';
            setSymptoms(trimmedBase + separator + trimmedNew);
          } else {
            setSymptoms(trimmedNew);
          }
        }
      };
      recognition.onerror = (event: any) => {
        setListening(false);
        setInterimTranscript('');
        setVoiceRecognitionInstance(null);
        const errorMessages: Record<string, string> = {
          'not-allowed': 'Unable to access the microphone. Please allow microphone access in your browser settings, then try again.',
          'permission-denied': 'Microphone permission was denied. Please allow access and try again.',
          ...
          'language-not-supported': 'The selected language is not supported for voice input in this browser. Please try English.',
          'aborted': '',
          'service-not-allowed': 'Speech recognition service is not allowed. Please use HTTPS or localhost.',
        };
        const msg = errorMessages[event.error] ?? `Voice recognition error: ${event.error}. Please type your symptoms manually.`;
      ...
      try { recognition.start(); }
        ...
        setVoiceError('Unable to start voice input. Please type your symptoms manually.');
        setListening(false);
        setVoiceRecognitionInstance(null);
      }
    };
    // Clean up recognition session if component unmounts or tab changes
    useEffect(() => {
      return () => {
        if (voiceRecognitionInstance) { try { voiceRecognitionInstance.stop(); } catch {} }
      };
    ...
    return <>
      ...
      {tab==='assistant'&&<section className="patient-card form-card patient-form-card intake"><p className="eyebrow">SwasthyaSetu AI</p><h2>AI Assistant</h2><p className="form-intro">Your care companion for understanding your health journey.</p><div className="ai-safe"><AlertTriangle/><span><b>For guidance only.</b> Medical decisions, diagnosis and emergency care must be handled by a qualified clinician.</span></div><div style={{ marginTop: 20 }}><label style={{ display: 'block', fontWeight: 'bold', marginBottom: '8px' }}>Describe your symptoms</label><div style={{ position: 'relative' }}><textarea id="voice-symptom-textarea" value={listening && interimTranscript ? (symptoms ? symptoms + '\n' + interimTranscript : interimTranscript) : symptoms} onChange={e => { if (!listening) setSymptoms(e.target.value); }} placeholder="e.g. I have had a headache for 2 days and a slight fever..." aria-label="Describe your symptoms" style={{ width: '100%', minHeight: '120px', padding: '12px 48px 12px 12px', border: listening ? '2px solid #ef4444' : '1px solid #e2e8f0', borderRadius: '8px', marginBottom: '0', boxSizing: 'border-box', resize: 'vertical', fontFamily: 'inherit', fontSize: '1em', lineHeight: 1.6, color: listening && interimTranscript ? '#94a3b8' : 'inherit', transition: 'border-color 0.2s' }} /><span style={{ position: 'absolute', bottom: '10px', right: '12px', fontSize: '18px', pointerEvents: 'none', opacity: 0.4 }} title={voiceSupported ? (listening ? 'Listening…' : 'Click microphone to speak') : 'Voice input not supported'}>{voiceSupported ? (listening ? '🔴' : '🎙️') : '🚫'}</span></div>{voiceError && <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginTop: 8, padding: '10px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, color: '#b91c1c', fontSize: '0.875em' }}><MicOff size={16} style={{ flexShrink: 0, marginTop: 1 }}/><span>{voiceError}</span></div>}{/* Convert-to-script hint: show when Indic language selected and textarea has Roman text */}{hasNativeScript(language) && symptoms.trim() && !isInScript(symptoms, LANGUAGE_CONFIG[language]?.scriptRanges ?? []) && <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, padding: '8px 12px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, fontSize: '0.85em', color: '#1d4ed8' }}><Languages size={14}/><span>Text appears to be in Roman script. Click to convert to {LANGUAGE_CONFIG[language]?.script} script.</span><button type="button" style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: 6, border: '1px solid #3b82f6', background: '#3b82f6', color: '#fff', fontSize: '0.82em', cursor: 'pointer', flexShrink: 0 }} onClick={() => { const normalized = normalizeForLanguage(symptoms, language); if (normalized !== symptoms) setSymptoms(normalized); }}>Convert</button></div>}<div className="actions" style={{ marginTop: 14 }}>{voiceSupported ? (<button type="button" id="voice-input-btn" className={listening ? 'outline voice-btn-listening' : 'outline'} onClick={toggleVoiceInput} aria-label={listening ? 'Stop voice input' : 'Start voice input'} title={listening ? 'Click to stop recording' : `Click to speak in ${getSpeechCode(language)}`} style={{ display: 'flex', alignItems: 'center', gap: 6, borderColor: listening ? '#ef4444' : undefined, color: listening ? '#ef4444' : undefined }}>{listening ? <><MicOff size={16}/> Stop recording</> : <><Mic size={16}/> Speak</>}</button>) : (<button type="button" className="outline" disabled title="Voice input is not supported in this browser" aria-disabled="true" style={{ display: 'flex', alignItems: 'center', gap: 6, opacity: 0.5, cursor: 'not-allowed' }}><MicOff size={16}/> Voice unavailable</button>)}<button className="primary" disabled={symptomAnalyzing || !(listening ? (symptoms || interimTranscript) : symptoms).trim()} onClick={analyzePatientSymptoms}>{symptomAnalyzing ? 'Analyzing...' : 'Analyze symptoms'} <Zap size={16} style={{ marginLeft: '8px' }}/></button><button className="outline" onClick={()=>setTab('records')}>View my health record <ChevronRight/></button></div></div>{symptomAnalysis && (<div style={{ marginTop: 30, padding: 20, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}><h3 style={{ marginTop: 0, marginBottom: 15, color: '#334155' }}>AI Assessment</h3><p style={{ margin: '8px 0' }}><b>Urgency:</b> {symptomAnalysis.triage?.urgency || 'Not available'}</p><div style={{ margin: '8px 0' }}><b>Red flags:</b> {symptomAnalysis.triage?.red_flags?.length ? <ul style={{ margin: '4px 0', paddingLeft: 20 }}>{symptomAnalysis.triage.red_flags.map((rf: any, i: number) => <li key={i}>{rf}</li>)}</ul> : ' None identified'}</div><div style={{ margin: '8px 0' }}><b>Summary:</b> <ul style={{ margin: '4px 0', paddingLeft: 20 }}>{Array.isArray(symptomAnalysis.summary) ? symptomAnalysis.summary.map((s: any, i: number) => <li key={i}>{s}</li>) : <li key="0">{(typeof symptomAnalysis.summary === 'string' ? symptomAnalysis.summary.replace(/^\s*\*\*Summary:\*\*\s*/i, '').trim() : symptomAnalysis.summary) || 'No summary available.'}</li>}</ul></div><div style={{ margin: '8px 0' }}><b>Suggestions:</b> {symptomAnalysis.suggestions?.length ? <ul style={{ margin: '4px 0', paddingLeft: 20 }}>{symptomAnalysis.suggestions.map((s: any, i: number) => <li key={i}>{s}</li>)}</ul> : ' No specific suggestions provided.'}</div><div style={{ marginTop: 15, padding: '12px', background: '#fff', borderLeft: '4px solid #f59e0b', fontSize: '0.9em', color: '#64748b' }}>AI-generated assessment. Human/clinician review is required. This is not a diagnosis.</div></div>)}</section>}
