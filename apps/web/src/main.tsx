import { useEffect, useState } from 'react'; import { createRoot } from 'react-dom/client'; import { Activity, AlertTriangle, Bell, Calendar, CheckCircle2, ChevronDown, ChevronRight, ChevronUp, ClipboardList, Clock, ExternalLink, FileText, Filter, FlaskConical, HeartPulse, Hospital, Languages, LogOut, MapPin, Menu, MessageSquare, Mic, MicOff, Navigation, Package, PackageCheck, Pencil, Phone, PhoneCall, Pill, Plus, Printer, RefreshCw, Send, ShieldAlert, Siren, Stethoscope, Trash2, Upload, User, UserCheck, UserCircle, Users, Video, VideoOff, Volume2, X, Zap } from 'lucide-react';
...
function App() {
    const initial = routeFromHash(); const [data, setData] = useState<Data>(); const [role, setRole] = useState<Role>(initial.role); const [tab, setTab] = useState(initial.tab); const [authPage, setAuthPage] = useState(authFromHash()); const [session, setSession] = useState<any>(() => { try { return JSON.parse(localStorage.getItem('swasthya-session') || 'null') } catch { return null } }); const [notice, setNotice] = useState(''); useEffect(() => { if (!notice) return; const timer = window.setTimeout(() => setNotice(''), 10000); return () => window.clearTimeout(timer); }, [notice]); const [profileMenuOpen, setProfileMenuOpen] = useState(false); const [profileEditOpen, setProfileEditOpen] = useState(false); const [profileSaving, setProfileSaving] = useState(false); const [language, setLanguage] = useState(() => {
      try {
        const savedSession = JSON.parse(localStorage.getItem('swasthya-session') || 'null');
        const sessionLanguage = String(savedSession?.language || '').toUpperCase();
        if (sessionLanguage === 'HI' || sessionLanguage === 'TA' || sessionLanguage === 'EN') return sessionLanguage;
        if (savedSession?.id) {
          const userLanguage = String(localStorage.getItem(`swasthya-language:${savedSession.id}`) || '').toUpperCase();
          if (userLanguage === 'HI' || userLanguage === 'TA' || userLanguage === 'EN') return userLanguage;
        }
      } catch {}
      return localStorage.getItem('swasthya-language') || 'EN';
    }); const [profileDraft, setProfileDraft] = useState<any>({ name:'', email:'', phone:'', village:'', language:localStorage.getItem('swasthya-language') || 'EN' }); const [menuOpen, setMenuOpen] = useState(false); useEffect(() => { const run = () => applyGlobalLanguage(language); const id = window.setTimeout(run, 0); const observer = new MutationObserver(run); if (language !== 'EN') observer.observe(document.body, { childList:true, subtree:true, characterData:true, attributes:true, attributeFilter:['placeholder','aria-label','title'] }); return () => { window.clearTimeout(id); observer.disconnect(); }; }, [language, tab, role, data]); const cacheKey = session?.patientId ? `swasthya-patient-cache-${session.patientId}` : '';
    ...
    const [apiError, setApiError] = useState(''); const refresh = async () => { try { const r = await fetch(`${api}/bootstrap`, { headers: session?.id ? { 'x-demo-user-id': session.id } : {}, cache:'no-store' }); if (!r.ok) throw new Error(`API returned ${r.status}`); const payload = await r.json(); const merged = mergeCachedPatient(payload); setData(merged); cachePatientData(merged); setApiError(''); return merged; } catch (e:any) { const message = e?.message === 'Failed to fetch' ? 'Cannot connect to the API server on port 4000. Start the API with: cd apps/api && npm run dev' : (e?.message || 'Unable to connect to the API server.'); setApiError(message); return null; } }; useEffect(() => { if (!location.hash) location.hash = '/login'; if (session) { if (session.patientId) { try { const cached=JSON.parse(localStorage.getItem(`swasthya-patient-cache-${session.patientId}`)||'null'); if (cached?.data) setData(cached.data); } catch {} } void refresh(); } const onHash = () => { setAuthPage(authFromHash()); const next = routeFromHash(); setRole(next.role); setTab(next.tab) }; addEventListener('hashchange', onHash); return () => removeEventListener('hashchange', onHash) }, [session]); useEffect(() => { if (session?.role !== 'health_worker' && session?.role !== 'patient') return; const timer = window.setInterval(() => { void refresh(); }, 5000); return () => window.clearInterval(timer); }, [session?.role]); if (authPage === 'login' || authPage === 'register') return <AuthPage mode={authPage} onSuccess={(next: any) => {
        const nextLanguage = String(next?.language || '').toUpperCase();
        const finalLanguage = nextLanguage === 'HI' || nextLanguage === 'TA' || nextLanguage === 'EN' ? nextLanguage : (localStorage.getItem('swasthya-language') || 'EN');
        const sessionWithLanguage = { ...next, language: finalLanguage };
        localStorage.setItem('swasthya-session', JSON.stringify(sessionWithLanguage));
        if (next?.id) localStorage.setItem(`swasthya-language:${next.id}`, finalLanguage);
        localStorage.setItem('swasthya-language', finalLanguage);
        setLanguage(finalLanguage);
        setSession(sessionWithLanguage);
        location.hash = `/${next.role}/home`;
        setAuthPage('');
      }} />; if (!session) { location.hash = '/login'; return <main className="loading"><HeartPulse /> Redirecting to secure sign in…</main> } if (role !== session.role) { location.hash = `/${session.role}/home`; return <main className="loading"><HeartPulse /> Opening your authorised workspace…</main> } if (!data) return <main className="loading"><HeartPulse /><strong>Unable to load the care network</strong><p>{apiError || 'Connecting to the API server…'}</p><button className="primary small" onClick={() => void refresh()}>Retry connection</button><small className="muted">API: http://localhost:4000/api/health</small></main>;
    const patient = session.role === 'patient' ? (data.patients || []).find((x: any) => x.id === session.patientId) : null; if (session.role === 'patient' && !patient) return <main className="loading"><HeartPulse /> Patient record unavailable. Please sign in again.</main>; const navigate = (nextRole: Role, nextTab = 'home') => { if (nextRole !== session.role) { setNotice('Your account does not have permission to open that workspace.'); return } location.hash = `/${nextRole}/${nextTab}`; setRole(nextRole); setTab(nextTab); setMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }) }; const action = async (path: string, body: any, message = 'Saved securely. Your care team can now continue this journey.') => { const response = await fetch(api + path, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); if (!response.ok) { const error = await response.json().catch(() => ({})); throw new Error(error.message || 'Unable to save'); } setNotice(message); await refresh() }; const chooseRole = (value: Role) => navigate(value); const signOut = () => { localStorage.removeItem('swasthya-session'); setSession(null); setProfileMenuOpen(false); setProfileEditOpen(false); location.hash = '/login'; setAuthPage('login') }; const profilePatient = session.role === 'patient' ? patient : null; const saveProfile = async () => { setProfileSaving(true); try { const userRes = await fetch(`${api}/users/${encodeURIComponent(session.id)}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ name:profileDraft.name, email:profileDraft.email }) }); const userResult = await userRes.json().catch(()=>({})); if (!userRes.ok) throw new Error(userResult.message || 'Unable to update profile.'); if (session.role === 'patient' && profilePatient) { const patientRes = await fetch(`${api}/patients/${encodeURIComponent(profilePatient.id)}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ name:profileDraft.name, village:profileDraft.village, phone:profileDraft.phone, language:profileDraft.language }) }); const patientResult = await patientRes.json().catch(()=>({})); if (!patientRes.ok) throw new Error(patientResult.message || 'Unable to update patient profile.'); } const selectedLanguage = ['EN','HI','TA'].includes(String(profileDraft.language || '').toUpperCase()) ? String(profileDraft.language).toUpperCase() : language; const nextSession = { ...session, name:profileDraft.name, email:profileDraft.email, language:selectedLanguage }; localStorage.setItem('swasthya-session', JSON.stringify(nextSession)); if (session.id) localStorage.setItem(`swasthya-language:${session.id}`, selectedLanguage); localStorage.setItem('swasthya-language', selectedLanguage); setLanguage(selectedLanguage); setSession(nextSession); await refresh(); setProfileEditOpen(false); setProfileMenuOpen(false); setNotice('Profile successfully updated.'); } catch (e:any) { setNotice(e.message || 'Unable to update profile.'); } finally { setProfileSaving(false); } };
    return <div className="app"><header><button className="brand" onClick={() => chooseRole(session.role)} aria-label="Go to authorised home"><span><HeartPulse /></span><div>Swasthya <b>Setu</b><small>Connecting care, everywhere</small></div></button><div className="header-actions"><div className="profile-menu-wrap"><button className="header-profile-button" onClick={()=>setProfileMenuOpen(x=>!x)} aria-expanded={profileMenuOpen}><span className="header-avatar">{session.name?.[0] || 'A'}</span><span className="header-profile-copy"><b>{session.name || 'Account'}</b><small>{session.role.replaceAll('_',' ')}</small></span><ChevronRight className={cn('profile-chevron',profileMenuOpen&&'open')}/></button>{profileMenuOpen&&<div className="profile-dropdown"><div className="profile-dropdown-head"><span className="profile-dropdown-avatar">{session.name?.[0] || 'A'}</span><div><b>{session.name || 'Account'}</b><small>{session.email || 'Signed-in account'}</small></div></div><button className="profile-dropdown-action" onClick={()=>{const currentPatient=session.role==='patient'?(data?.patients||[]).find((x:any)=>x.id===session.patientId):null;setProfileDraft({name:session.name||'',email:session.email||'',phone:currentPatient?.phone||'',village:currentPatient?.village||'',language});setProfileEditOpen(true);setProfileMenuOpen(false)}}><Pencil/><span><b>Edit profile</b><small>Update your personal details</small></span></button><div className="profile-dropdown-divider"/><button className="profile-signout" onClick={signOut}><LogOut/><span>Sign out</span></button></div>}</div><button className="mobile" onClick={() => setMenuOpen(x => !x)} aria-label="Open navigation"><Menu /></button></div></header>{notice && <div className="notice"><CheckCircle2 /> {notice}<button onClick={() => setNotice('')}>×</button></div>}
        {profileEditOpen && <div className="modal-backdrop" onClick={()=>!profileSaving&&setProfileEditOpen(false)}><section className="profile-modal" onClick={e=>e.stopPropagation()}><div className="card-heading"><div><p className="eyebrow">Profile</p><h3>Edit your profile</h3><p className="form-intro">Keep your account and care-contact details up to date.</p></div><button className="icon-close" onClick={()=>!profileSaving&&setProfileEditOpen(false)}>×</button></div><div className="profile-fields"><label>Full name<input value={profileDraft.name} onChange={e=>setProfileDraft({...profileDraft,name:e.target.value})}/></label><label>Email address<input type="email" value={profileDraft.email} onChange={e=>setProfileDraft({...profileDraft,email:e.target.value})}/></label>{session.role==='patient'&&<><label>Mobile number<input value={profileDraft.phone} onChange={e=>setProfileDraft({...profileDraft,phone:e.target.value})}/></label><label>Age<input value={profilePatient?.age ?? ''} readOnly/></label><label>Village / locality<input value={profileDraft.village} onChange={e=>setProfileDraft({...profileDraft,village:e.target.value})}/></label><label className="profile-language-field"><span>Language</span><select value={profileDraft.language || language} onChange={e=>setProfileDraft({...profileDraft,language:e.target.value})}><option value="EN">English</option><option value="HI">हिन्दी</option><option value="TA">தமிழ்</option></select><small>Choose your preferred language for the patient dashboard.</small></label></>}</div><button className="primary" disabled={profileSaving} onClick={()=>void saveProfile()}>{profileSaving?'Saving…':'Save profile'}</button></section></div>} {role === 'patient' ? <PatientView p={patient} data={data} tab={tab} setTab={(next: string) => navigate('patient', next)} action={action} setNotice={setNotice} refresh={refresh} language={language} setLanguage={(v:string)=>{setLanguage(v);localStorage.setItem('swasthya-language',v)}} profileOpen={profileEditOpen} setProfileOpen={setProfileEditOpen} /> : role === 'doctor' ? <DoctorView data={data} action={action} setNotice={setNotice} refresh={refresh} language={language} /> : <AdminView data={data} role={role} facilityId={session.facilityId} setNotice={setNotice} refresh={refresh} />}</div>
}
function AuthPage({ mode, onSuccess }: { mode: 'login' | 'register'; onSuccess: (user: any) => void }) {
  const [role, setRole] = useState<Role>('patient');
  const [name, setName] = useState(''); const [email, setEmail] = useState(''); const [password, setPassword] = useState('');
  const [phone, setPhone] = useState(''); const [village, setVillage] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  const [language, setLanguage] = useState(() => localStorage.getItem('swasthya-language') || 'EN');
  const tr = (key: string) => GLOBAL_TRANSLATIONS[language]?.[key] || key;
  const changeLanguage = (value: string) => { setLanguage(value); localStorage.setItem('swasthya-language', value); };
  const labels: { role: Role; label: string; detail: string }[] = [
    { role: 'patient', label: tr('Patient'), detail: tr('Access your care and records') },
    { role: 'health_worker', label: tr('Health worker'), detail: tr('Support community care') },
    { role: 'doctor', label: tr('Doctor / specialist'), detail: tr('Review and consult') }
  ];
  const demo: any = { patient: ['meera@example.com', 'demo123'], health_worker: ['worker@example.com', 'demo123'], doctor: ['doctor@example.com', 'demo123'] };
  const submit = async (e: any) => { e.preventDefault(); setError(''); setLoading(true); try { const response = await fetch(`${api}/auth/${mode}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ role, name, email, password, phone, village, language }) }); const result = await response.json(); if (!response.ok) throw new Error(result.message || 'Unable to continue.'); const serverLanguage = String(result.user?.language || '').toUpperCase();
      const finalLanguage = serverLanguage === 'HI' || serverLanguage === 'TA' || serverLanguage === 'EN' ? serverLanguage : language;
      localStorage.setItem('swasthya-language', finalLanguage);
      if (result.user?.id) localStorage.setItem(`swasthya-language:${result.user.id}`, finalLanguage);
      onSuccess({ ...result.user, language: finalLanguage }) } catch (err: any) { setError(err.message) } finally { setLoading(false) } };
  const useDemo = () => { setEmail(demo[role][0]); setPassword(demo[role][1]); setName('') };
  return <main className="auth-shell">
    <section className="auth-aside">
      <div className="auth-brand"><HeartPulse /> Swasthya <b>Setu</b></div>
      <p className="eyebrow">{tr('Integrated rural healthcare')}</p>
      <h1>{tr('One connected path to ')}<i>{tr('better care.')}</i></h1>
      <p>{tr('Securely connect patients, community health workers, clinicians, and facilities around the same care record.')}</p>
      <div className="auth-steps"><span><CheckCircle2 /> {tr('Consent-led access')}</span><span><CheckCircle2 /> {tr('Human-reviewed AI support')}</span><span><CheckCircle2 /> {tr('Care continuity across facilities')}</span></div>
    </section>
    <section className="auth-card">
      {mode === 'login' && <div className="auth-language-switcher"><Languages /><span>{tr('Language')}</span><select aria-label={tr('Language')} value={language} onChange={e => changeLanguage(e.target.value)}><option value="EN">English</option><option value="HI">हिन्दी</option><option value="TA">தமிழ்</option></select></div>}
      <div className="auth-card-head"><p className="eyebrow">{tr(mode === 'login' ? 'Welcome back' : 'Create an account')}</p><h2>{tr(mode === 'login' ? 'Sign in to your care portal' : 'Join the connected care network')}</h2><p>{tr(mode === 'login' ? 'Choose your stakeholder role before signing in.' : 'Tell us how you will use Swasthya Setu.')}</p></div>
      <form onSubmit={submit}><label>{tr('I am a')}</label><div className="role-picker">{labels.map(x => <button type="button" className={cn(role === x.role && 'active')} onClick={() => setRole(x.role)} key={x.role}><b>{x.label}</b><small>{x.detail}</small></button>)}</div>
      {mode === 'register' && <><label>{tr('Full name')}</label><input required value={name} onChange={e => setName(e.target.value)} placeholder={tr('Your full name')} />{role === 'patient' && <div className="form-row"><div><label>{tr('Mobile number')}</label><input required value={phone} onChange={e => setPhone(e.target.value)} placeholder={tr('10-digit mobile')} /></div><div><label>{tr('Village / locality')}</label><input required value={village} onChange={e => setVillage(e.target.value)} placeholder={tr('Your village')} /></div></div>}</>}
      <label>{tr('Email address')}</label><input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="name@example.com" /><label>{tr('Password')}</label><input required type="password" minLength={6} value={password} onChange={e => setPassword(e.target.value)} placeholder={tr('At least 6 characters')} />
      {error && <p className="auth-error"><AlertTriangle /> {error}</p>}
      <button className="primary auth-submit" disabled={loading}>{loading ? tr('Please wait…') : tr(mode === 'login' ? 'Sign in securely' : 'Create secure account')} <ChevronRight /></button></form>
      {mode === 'login' && <button className="demo-link" onClick={useDemo}>{tr('Use demo account for selected role')}</button>}
      <p className="auth-switch">{tr(mode === 'login' ? 'New to Swasthya Setu?' : 'Already have an account?')} <button onClick={() => location.hash = mode === 'login' ? '/register' : '/login'}>{tr(mode === 'login' ? 'Create an account' : 'Sign in')}</button></p>
    </section>
  </main>
}

// Localised UI strings shown inside the care-team chat
const CARE_UI_STRINGS: Record<string, Record<string, string>> = {
  EN: {
    sendTo: 'Send to',
    entireCareTeam: 'Entire Care Team',
    patient: 'Patient',
    doctor: 'Doctor',
    healthWorker: 'Health Worker',
    noMsgTitle: 'No messages yet',
    noMsgBody: 'Send an update. Messages to the patient are automatically converted into their language.',
    send: 'Send',
    sending: 'Sending…',
    changeLanguage: 'Change language:',
    bannerNote: 'Doctor and healthcare worker messages will automatically appear in this language.',
    placeholderPatient: (lang: string) => `Write or speak your message in ${lang} (translated to English for your doctor)…`,
    footerPatient: 'Your message will be sent to the healthcare team and translated to English.',
    listen: 'Listen',
  },
  HI: {
    sendTo: 'भेजें',
    entireCareTeam: 'पूरी केयर टीम',
    patient: 'मरीज़',
    doctor: 'डॉक्टर',
    healthWorker: 'स्वास्थ्य कार्यकर्ता',
    noMsgTitle: 'अभी कोई संदेश नहीं',
    noMsgBody: 'एक अपडेट भेजें। आपके संदेश स्वचालित रूप से डॉक्टर को अंग्रेज़ी में अनुवाद होकर जाएंगे।',
    send: 'भेजें',
    sending: 'भेजा जा रहा है…',
    changeLanguage: 'भाषा बदलें:',
    bannerNote: 'डॉक्टर और स्वास्थ्य कार्यकर्ता के संदेश स्वचालित रूप से इस भाषा में दिखेंगे।',
    placeholderPatient: (lang: string) => `${lang} में लिखें या बोलें (आपके डॉक्टर के लिए अंग्रेज़ी में अनुवाद होगा)…`,
    footerPatient: 'आपका संदेश स्वास्थ्य टीम को भेजा जाएगा और अंग्रेज़ी में अनुवाद किया जाएगा।',
    listen: 'सुनें',
  },
  TA: {
    sendTo: 'அனுப்பு',
    entireCareTeam: 'முழு கவலை குழு',
    patient: 'நோயாளி',
    doctor: 'மருத்துவர்',
    healthWorker: 'சுகாதார பணியாளர்',
    noMsgTitle: 'இன்னும் செய்திகள் இல்லை',
    noMsgBody: 'ஒரு புதுப்பிப்பை அனுப்புங்கள். உங்கள் செய்திகள் தானாக ஆங்கிலத்தில் மொழிபெயர்க்கப்பட்டு மருத்துவரிடம் அனுப்பப்படும்.',
    send: 'அனுப்பு',
    sending: 'அனுப்புகிறது…',
    changeLanguage: 'மொழியை மாற்று:',
    bannerNote: 'மருத்துவர் மற்றும் சுகாதார பணியாளரின் செய்திகள் தானாக இந்த மொழியில் தோன்றும்.',
    placeholderPatient: (lang: string) => `${lang} மொழியில் எழுதுங்கள் அல்லது பேசுங்கள் (மருத்துவருக்கு ஆங்கிலத்தில் மொழிபெயர்க்கப்படும்)…`,
    footerPatient: 'உங்கள் செய்தி சுகாதார குழுவிற்கு அனுப்பப்பட்டு ஆங்கிலத்தில் மொழிபெயர்க்கப்படும்.',
    listen: 'கேளுங்கள்',
  },
};

function CareTeamMessages({ patientId, data, setNotice, refresh, actorRole, actorId, actorName, compact = false, language, setLanguage }: any) {
  ...
  // Determine active patient language preference
  const rawPatientLang = String(patient?.language || 'Hindi').toUpperCase();
  const patientLangCode = (rawPatientLang === 'TAMIL' || rawPatientLang === 'TA') ? 'TA' : (rawPatientLang === 'ENGLISH' || rawPatientLang === 'EN') ? 'EN' : 'HI';
  const activeLanguage = actorRole === 'patient' ? (language || patientLangCode) : patientLangCode;

  // UI locale: patient sees text in their language; clinicians always see English
  const uiLang = actorRole === 'patient' ? (activeLanguage as 'EN' | 'HI' | 'TA') : 'EN';
  const t = (key: string) => (CARE_UI_STRINGS[uiLang] || CARE_UI_STRINGS.EN)[key] ?? key;

  // Deduplicate and filter messages for this patient
  const rawMessages: any[] = (data.careTeamMessages || []).filter((m:any) => m.patientId === activePatientId);
  const seenKeys = new Set<string>();
  const messages: any[] = rawMessages.filter((m: any) => {
    const key = `${m.createdAt}_${m.senderRole}_${(m.originalMessage || m.message || '').slice(0, 40)}`;
    if (seenKeys.has(key)) return false;
    seenKeys.add(key);
    return true;
  });

  const [recipientRole, setRecipientRole] = useState<'care_team'|'patient'|'health_worker'|'doctor'>(actorRole === 'patient' ? 'care_team' : 'patient');
  ...
  const langLabel = (code: string) => code === 'HI' ? 'हिन्दी (Hindi)' : code === 'TA' ? 'தமிழ் (Tamil)' : 'English';

  const switchPatientLanguage = async (newLang: string) => {
    try {
      setNotice(`Switching patient language to ${langLabel(newLang)}…`);
      const res = await fetch(`${api}/patients/${encodeURIComponent(activePatientId)}`, {
        method: 'PATCH',
      ...
      localStorage.setItem('swasthya-language', newLang);
      await refresh?.();
      setNotice(`Language switched to ${langLabel(newLang)}. Messages will be automatically converted to this language.`);
    } catch (e: any) {
      setNotice(e?.message || 'Unable to update language.');
    }
  };
  ...
  const toggleSpeech = () => {
    ...
    if (!SpeechRecognition) {
      setNotice('Voice input is unavailable in this browser; you can still type your message.');
      return;
    }
    try {
      const recognition = new SpeechRecognition();
      const targetLang = actorRole === 'patient'
        ? (activeLanguage === 'HI' ? 'hi-IN' : activeLanguage === 'TA' ? 'ta-IN' : 'en-IN')
        : 'en-IN';
      ...
      recognition.onstart = () => {
        setListening(true);
        setNotice(`Listening in ${langLabel(activeLanguage)}… speak now.`);
      };
      ...
      recognition.onresult = (event: any) => {
        ...
        if (transcript) {
          ...
          setNotice('Voice message captured.');
        }
      };
      recognition.start();
    } catch {
      setListening(false);
      setNotice('Could not start microphone.');
    }
  };
  ...
  const playAudio = (text: string, langCode: string) => {
    ...
    u.lang = langCode === 'HI' ? 'hi-IN' : langCode === 'TA' ? 'ta-IN' : 'en-IN';
    u.rate = 0.95;
  ...
  const send = async () => {
    ...
    try {
      ...
      const r = await fetch(`${api}/care-team/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      setMessage('');
      await refresh?.();
      const translationNote = result.translationStatus === 'translated'
        ? (actorRole === 'patient'
            ? ' (translated to English for clinician)'
            : ` (automatically translated to ${langLabel(result.patientLanguage)} for patient)`)
        : '';
      setNotice(`Message delivered${translationNote}.`);
    } catch (e: any) {
      setNotice(e.message || 'Unable to send message.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return <section className={cn('care-team-card', compact && 'care-team-card-compact')}>
    <div className="care-team-head">
      <div>
        <p className="eyebrow">Multilingual Continuity of Care</p>
        <h3><MessageSquare/> Care Team Chat</h3>
        <p>Three-way communication · Patient ↔ Frontline Worker ↔ Doctor</p>
      </div>
      <div className="care-team-status-group">
        <span className="care-team-status"><span></span>Live &amp; Auto-Translating</span>
      </div>
    </div>

    {/* Patient Language Bar */}
    <div className="care-language-banner">
      <div className="care-language-info">
        <Languages style={{ width: 18, color: '#0f766e' }} />
        <div>
          <b>Patient Selected Language: <span className="care-lang-pill">{langLabel(activeLanguage)}</span></b>
          <small>
            {actorRole === 'patient'
              ? t('bannerNote')
              : `Messages you send are automatically translated into ${langLabel(activeLanguage)} for ${patient.name}.`}
          </small>
        </div>
      </div>
      {actorRole === 'patient' && (
        <div className="care-lang-selector">
          <span style={{ fontSize: 11, fontWeight: 700, color: '#475569' }}>{t('changeLanguage')}</span>
          {(['HI', 'TA', 'EN'] as const).map(code => (
            <button
              key={code}
              type="button"
              className={cn('care-lang-btn', activeLanguage === code && 'active')}
              onClick={() => void switchPatientLanguage(code)}
            >
              {code === 'HI' ? 'हिन्दी' : code === 'TA' ? 'தமிழ்' : 'English'}
            </button>
          ))}
        </div>
      )}
    ...
    {actorRole !== 'patient' && (
      <label className="care-team-patient">
        <span>Patient</span>
        {actorRole === 'health_worker' ? (
          <select value={activePatientId} onChange={e => setConversationPatientId(e.target.value)}>
            {patients.map((p: any) => (
              <option key={p.id} value={p.id}>{p.name} · {p.age} years ({p.language || 'Hindi'})</option>
            ))}
          </select>
        ) : (
          <select value={activePatientId} disabled><option>{patient.name} ({patient.language || 'Hindi'})</option></select>
        )}
      </label>
    )}

    <div className="care-shared-context">
      <div><b>Shared clinical context</b><span>Available to patient, assigned frontline worker and doctor.</span></div>
      <div className="care-shared-pills">
        <span>{patient.medicalCondition || 'Condition not recorded'}</span>
        <span>{patient.medications ? 'Medicines available' : 'No medicines recorded'}</span>
        <span>{documents.length} shared report{documents.length === 1 ? '' : 's'}</span>
      </div>
    </div>
    ...
    <div className="care-shared-documents" style={{margin:'12px 0',padding:12,border:'1px solid #dbeafe',borderRadius:12,background:'#f8fbff'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,marginBottom:8}}>
        <div><b style={{fontSize:13}}>Shared medical reports</b><span style={{display:'block',fontSize:11,color:'#64748b'}}>Open medical records shared for this consultation.</span></div>
        <span className="tag">{documents.length} shared</span>
      </div>
      {documents.length ? documents.map((doc:any)=>(
        <div key={doc.id} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 0',borderTop:'1px solid #e2e8f0'}}>
          <FileText style={{width:16,color:'#2563eb'}}/>
          <div style={{flex:1,minWidth:0}}>
            <b style={{fontSize:12,display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{doc.name}</b>
            <small style={{color:'#64748b'}}>{doc.type.replaceAll('_',' ')} · {formatBytes(doc.size)}</small>
          </div>
          <button className="outline small" onClick={()=>openMedicalDocument(doc)}>Open PDF / report</button>
        </div>
      )) : <small style={{color:'#64748b'}}>No shared medical reports uploaded yet.</small>}
    </div>

    <div className="care-call-bar">
      <div><b><Video/> Telecommunication Room</b><span>Secure 3-way audio/video consult.</span></div>
      <button className="primary" onClick={openCareCall}><Video/> {callOpen ? 'Join care call' : 'Start 3-way call'}</button>
    </div>

    {/* Messages thread */}
    <div className="care-team-thread">
      {messages.length ? messages.slice(-16).map((m: any) => {
        const isMine = m.senderRole === actorRole;
        const isFromPatient = m.senderRole === 'patient';
        const isPatientViewer = actorRole === 'patient';
        const msgLang = m.patientLanguage || activeLanguage;

        // Determine what text to display prominently
        let mainText = m.message;
        let altText = '';
        let altLabel = '';
        let badge = null;
        ...
        if (isPatientViewer) {
          if (isFromPatient) {
            mainText = m.originalMessage || m.message;
            badge = <span className="care-translation-badge care-badge-sent"><CheckCircle2 size={10}/> Sent to care team (translated to English)</span>;
          } else {
            mainText = m.translatedMessage || m.message;
            if (m.originalMessage && m.originalMessage !== mainText) {
              altText = m.originalMessage;
              altLabel = 'Original English message';
              badge = <span className="care-translation-badge care-badge-translated"><Languages size={10}/> Auto-translated to {langLabel(msgLang)}</span>;
            }
          }
        } else {
          ...
          if (isFromPatient) {
            ...
            if (m.originalMessage && m.originalMessage !== mainText) {
              altText = m.originalMessage;
              altLabel = `Original patient words (${langLabel(msgLang)})`;
              badge = <span className="care-translation-badge care-badge-translated"><Languages size={10}/> Translated from {langLabel(msgLang)}</span>;
            }
          } else {
            mainText = m.originalMessage || m.message;
            if (m.translatedMessage && m.translatedMessage !== mainText) {
              altText = m.translatedMessage;
              altLabel = `Delivered to patient in ${langLabel(msgLang)}`;
              badge = <span className="care-translation-badge care-badge-delivered"><CheckCircle2 size={10}/> Auto-translated to {langLabel(msgLang)} for ${patient.name}</span>;
            }
          }
        }

        const isDetailOpen = !!expandedDetails[m.id];

        return (
          <article key={m.id} className={cn('care-message', isMine && 'mine', isFromPatient ? 'from-patient' : 'from-clinician')}>
            <div className="care-message-meta">
              <b>{m.senderName}</b>
              <span className={cn('role-tag', m.senderRole)}>{roleLabel(m.senderRole)}</span>
              <small>{new Date(m.createdAt).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</small>
            </div>

            <p className="care-message-body">{mainText}</p>

            <div className="care-message-foot">
              <small className="care-message-to">To {roleLabel(m.recipientRole)}</small>
              <div className="care-message-actions">
                <button
                  type="button"
                  className="care-tts-btn"
                  title="Listen to message audio"
                  onClick={() => playAudio(mainText, isPatientViewer ? msgLang : 'EN')}
                >
                  <Volume2 size={12} /> <span>{t('listen')}</span>
                </button>
                {altText && (
                  <button
                    type="button"
                  ...
                  >
                    {isDetailOpen ? 'Hide' : altLabel}
                  </button>
                )}
              </div>
            </div>

            {badge && <div className="care-message-badge-wrap">{badge}</div>}

            {isDetailOpen && altText && (
              <div className="care-detail-drawer">
                <small>{altLabel}:</small>
                <p>{altText}</p>
              </div>
            )}
          </article>
    ...
    {/* Compose row */}
    <div className="care-team-compose">
      <div className="care-team-recipient">
        <span>{t('sendTo')}</span>
        <div className="care-recipient-buttons">
          <button
            type="button"
            className={cn(recipientRole === 'care_team' && 'active')}
            onClick={() => setRecipientRole('care_team')}
          >
            {t('entireCareTeam')}
          </button>
          {(['patient', 'health_worker', 'doctor'] as const)
            .filter(x => x !== actorRole)
            .map(x => (
              <button
                type="button"
                key={x}
                className={cn(recipientRole === x && 'active')}
                onClick={() => setRecipientRole(x)}
              >
                {x === 'patient' ? t('patient') : x === 'doctor' ? t('doctor') : t('healthWorker')}
              </button>
            ))}
        </div>
      </div>

      <div className="care-compose-row">
        <div className="care-input-wrap">
          <textarea
            ...
            placeholder={
              actorRole === 'patient'
                ? (CARE_UI_STRINGS[uiLang]?.placeholderPatient as any)?.(langLabel(activeLanguage)) ?? `Write or speak your message in ${langLabel(activeLanguage)}…`
                : `Write your instructions in English (will automatically be converted to ${langLabel(activeLanguage)} for the patient)…`
            }
          />
          <button
            type="button"
            className={cn('care-mic-btn', listening && 'listening')}
            onClick={toggleSpeech}
            title={listening ? 'Stop listening' : 'Speak message (Voice-to-Text)'}
          >
            {listening ? <MicOff size={16} /> : <Mic size={16} />}
          </button>
        </div>
        <button className="primary care-send-btn" disabled={busy || !message.trim()} onClick={() => void send()}>
          <Send size={15} /> {busy ? t('sending') : t('send')}
        </button>
      </div>

      <div className="care-compose-footer">
        <small className="care-translation-hint">
          <Languages size={11} style={{ verticalAlign: 'middle', marginRight: 4 }} />
          {actorRole === 'patient'
            ? t('footerPatient')
            : `Delivers in ${langLabel(activeLanguage)} to ${patient.name} via Puter AI translation.`}
        </small>
        <small className="care-character-count">{message.length}/500</small>
      </div>
    </div>
  </section>;
}
...
function PatientView({ p, data, tab, setTab, action, setNotice, language, setLanguage, refresh, profileOpen, setProfileOpen }: any) {
    ...
    const analyzePatientSymptoms = async () => {
      const trimmed = symptoms.trim();
      if (!trimmed) {
        setNotice('Please enter your symptoms first.');
        return;
      }
      ...
      try {
        const res = await fetch(`${api}/ai/analyze/symptoms`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            symptoms: [{ symptom: trimmed, duration: 'Not specified', severity: 'Not specified' }]
          })
        });
        const result = await res.json().catch(() => ({}));
        if (res.status === 503) throw new Error('AI symptom analysis is currently unavailable.');
        if (!res.ok || !result.success) throw new Error(result.error || result.reason || 'AI analysis is currently unavailable');
        setSymptomAnalysis(result.data?.result ?? result.data);
        setNotice('AI assessment completed successfully.');
      } catch (err: any) {
        console.error('AI symptom analysis error:', err);
        let msg = err.message || 'AI analysis is currently unavailable.';
        if (msg.includes('fetch failed') || msg.includes('Failed to fetch')) {
          msg = 'AI symptom analysis is currently unavailable.';
        }
        setNotice(msg);
      } finally {
      ...
      try {
        ...
        if (['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
          try {
            ...
            const aiResp = await fetch(`${api}/ai/analyze/report`, { method: 'POST', body: formData });
            const aiData = await aiResp.json().catch(() => ({}));
            console.log('AI report analysis:', aiData);
            if (!aiResp.ok || !aiData.success) {
               setNotice('Report uploaded successfully. AI analysis is currently unavailable.');
            } else {
               setNotice('Report added to your medical history and analyzed by AI.');
            }
          } catch (aiErr) {
    ...
    return <>
      ...
      {tab==='assistant'&&<section className="patient-card form-card patient-form-card intake"><p className="eyebrow">SwasthyaSetu AI</p><h2>AI Assistant</h2><p className="form-intro">Your care companion for understanding your health journey.</p><div className="ai-safe"><AlertTriangle/><span><b>For guidance only.</b> Medical decisions, diagnosis and emergency care must be handled by a qualified clinician.</span></div><div style={{ marginTop: 20 }}><label style={{ display: 'block', fontWeight: 'bold', marginBottom: '8px' }}>Describe your symptoms</label><textarea value={symptoms} onChange={e => setSymptoms(e.target.value)} placeholder="e.g. I have had a headache for 2 days and a slight fever..." style={{ width: '100%', minHeight: '100px', padding: '12px', border: '1px solid #e2e8f0', borderRadius: '8px', marginBottom: '16px' }} /><div className="actions"><button className="primary" disabled={symptomAnalyzing || !symptoms.trim()} onClick={analyzePatientSymptoms}>{symptomAnalyzing ? 'Analyzing...' : 'Analyze symptoms'} <Zap size={16} style={{ marginLeft: '8px' }}/></button><button className="outline" onClick={()=>setTab('records')}>View my health record <ChevronRight/></button></div></div>{symptomAnalysis && (<div style={{ marginTop: 30, padding: 20, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}><h3 style={{ marginTop: 0, marginBottom: 15, color: '#334155' }}>AI Assessment</h3><p style={{ margin: '8px 0' }}><b>Urgency:</b> {symptomAnalysis.triage?.urgency || 'Not available'}</p><div style={{ margin: '8px 0' }}><b>Red flags:</b> {symptomAnalysis.triage?.red_flags?.length ? <ul style={{ margin: '4px 0', paddingLeft: 20 }}>{symptomAnalysis.triage.red_flags.map((rf: any, i: number) => <li key={i}>{rf}</li>)}</ul> : ' None identified'}</div><div style={{ margin: '8px 0' }}><b>Summary:</b> <ul style={{ margin: '4px 0', paddingLeft: 20 }}>{Array.isArray(symptomAnalysis.summary) ? symptomAnalysis.summary.map((s: any, i: number) => <li key={i}>{s}</li>) : <li key="0">{(typeof symptomAnalysis.summary === 'string' ? symptomAnalysis.summary.replace(/^\s*\*\*Summary:\*\*\s*/i, '').trim() : symptomAnalysis.summary) || 'No summary available.'}</li>}</ul></div><div style={{ margin: '8px 0' }}><b>Suggestions:</b> {symptomAnalysis.suggestions?.length ? <ul style={{ margin: '4px 0', paddingLeft: 20 }}>{symptomAnalysis.suggestions.map((s: any, i: number) => <li key={i}>{s}</li>)}</ul> : ' No specific suggestions provided.'}</div><div style={{ marginTop: 15, padding: '12px', background: '#fff', borderLeft: '4px solid #f59e0b', fontSize: '0.9em', color: '#64748b' }}>AI-generated assessment. Human/clinician review is required. This is not a diagnosis.</div></div>)}</section>}
...
function DoctorView({ data, action, setNotice, refresh, language }: any) {
  ...
  return (
    <main className="dashboard" style={{ paddingTop: 16 }}>
      ...
      {selectedCase && <CareTeamMessages patientId={selectedCase.patientId} data={data} setNotice={setNotice} refresh={refresh} actorRole="doctor" actorId="user-doctor" actorName="Dr. Ananya Sharma" compact language={language} />}
